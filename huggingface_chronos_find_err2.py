...
#print(f"異常點誤差: {errors[anomaly_indices]}")
#print(f"異常點真實值: {y_true[anomaly_indices]}")
#print(f"異常點預測值: {y_pred[anomaly_indices]}")

# 繪製預測與真實值對比圖，並標記異常點
plt.figure(figsize=(10, 6))
plt.plot(range(prediction_length), y_true, label='True Values')
plt.plot(range(prediction_length), y_pred, label='Predicted Values', alpha=0.7)
plt.scatter(anomaly_indices - offset, y_true[anomaly_indices - offset], color='red', label='Anomalies')
plt.xlabel('Time Steps')
plt.ylabel('Normalized Target Value')
plt.xlim(0, prediction_length - 1)  # 設置x軸範圍
plt.legend()
plt.title('True vs Predicted Values (with Anomalies) - Longitude')
plt.savefig("huggingface_chronos_find_err2.png")
